...
### Phase 2: Upload and Embed the Frontend (Using index.aspx)

> [!IMPORTANT]
> **Why we use index.aspx**:
> SharePoint Online has strict browser file handling security rules. If you upload a standard `.html` file, SharePoint serves it as a download (`Content-Disposition: attachment`), which prevents it from rendering inside an iframe.
> To bypass this, we use the unified single-file `index.aspx` file from your workspace. SharePoint natively executes and displays `.aspx` files in the browser. Since `index.aspx` is self-contained, you only need to upload this **one file**!

1. **Upload index.aspx to SharePoint**:
   ...
   * Upload the unified **`index.aspx`** file into this folder. (You do **not** need to upload `index.html`, `styles.css`, or `app.js` since `index.aspx` is fully self-contained).
2. **Get the URL of index.aspx**:
   * Click the three dots (`...`) next to the uploaded `index.aspx` $\rightarrow$ Click **Details** or **Copy link**.
   * Copy the direct URL. It should look like: `https://yourdomain.sharepoint.com/sites/WorkspaceServices/SiteAssets/Roadmap/index.aspx`.
3. **Embed the Roadmap on a Modern SharePoint Page**:
   ...
   * Paste the iframe snippet into the embed code box, using the URL you copied:
     ```html
     <iframe src="https://yourdomain.sharepoint.com/sites/WorkspaceServices/SiteAssets/Roadmap/index.aspx" width="100%" height="800px" style="border: 0; border-radius: 8px;"></iframe>
     ```
